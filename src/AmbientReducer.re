...
let create (a, parent, capability, cb): ambient = {
  ...
  | "" => let res = createInAmbient(source, target) -> updateChild(parent); res |> treeToString |> cb; res
  | _ => let res = createInRoot(source, target) -> updateChild(parent); res |> treeToString |> cb; res
  };
};
...
let enter (a, b, parent, capability, cocapability, cb): ambient = {
  ...
  let res = parent |> removeChild(source) |> updateChild(updated); res |> treeToString |> cb; res
};
...
let exit (a, b, parent, capability, cocapability, cb): ambient = {
  ...
  /* Remove the entering ambient from its parent */
  let res = parent |> addChild(source) |> updateChild(updated); res |> treeToString |> cb; res
};
...
let open_ (a, b, parent, capability, cocapability, cb): ambient = {
  ...
  | _ => updated |> treeToString |> cb; updated
  };
};
...
let rec applyTransitionsRecursive (ambient, cb: 'a => unit): ambient = {
  let reducer (res, child: ambient) = updateChild(applyTransitionsRecursive(child, cb), res);
...
let rec reduceFully (ambient) = {
  ...
  | true => applyTransitionsRecursive(transitionTree, print_string) |> reduceFully
  | false => ambient
  };
};
...
let rec reduceFullyDebug (index, cb: 'a => unit, ambient) = {
  ...
  | true => applyTransitionsRecursive(transitionTree, cb) |> reduceFullyDebug(index + 1, cb)
  | false => ambient
  };
};
